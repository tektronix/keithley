...
It uses the SMUs, CVU, and CVIV by calling the EX command. It plots the results of the
SMU and CVU tests.
...
Relies on plotly to plot the family of curves.
...
# PLOTLY COMMANDS
# Loop through each segment and add scatter + line
fig_smu = go.Figure()
for seg in range(1, 5):
    voltage_col = f"DrainV({seg})"
    current_col = f"DrainI({seg})"
    fig_seg = px.scatter(
        df_smu,
        x=voltage_col,
        y=current_col,
        labels={voltage_col: "Drain Voltage (V)", current_col: "Drain Current (A)"},
        title="n-MOSFET Drain Family",
        hover_data={voltage_col: True, current_col: True},
    )
    fig_smu.add_trace(fig_seg.data[0])  # scatter
    fig_smu.add_trace(px.line(df_smu, x=voltage_col, y=current_col).data[0])  # line

fig_smu.update_layout(
    xaxis_title="Drain Voltage (V)",
...
fig_cvu = px.scatter(
    ...
    labels={"Voltage (V)": "DCV_GB", "Capacitance (F)": "Cp_GB"},
    hover_data=df_cvu.columns,
)

# Add line trace
fig_cvu.add_trace(px.line(df_cvu, x="Voltage (V)", y="Capacitance (F)").data[0])
fig_cvu.show()
