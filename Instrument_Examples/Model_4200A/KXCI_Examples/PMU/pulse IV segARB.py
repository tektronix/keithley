...
#Configure an array of trigger values for sequence 1 of channel 1 of the pulse card
my4200.query(":PMU:SARB:SEQ:TRIG 1, 1, 1, 1, 1, 0, 0, 0, 0")
#Configure an array of output relay values for sequence 1 of channel 1 of the pulse card
my4200.query(":PMU:SARB:SEQ:SSR 1, 1, 1, 1, 1, 1, 1, 1, 0")
...
my4200.query(":PMU:SARB:SEQ:MEAS:START 1, 1, 0, 0, 0, 0, 0, 0, 0")
...
df = pd.DataFrame(coords2d, columns=['Voltage', 'Current', 'Timestamp', 'Status'])
...
df = df.astype({'Voltage': float, 'Current': float, 'Timestamp': str, 'Status': str})
...
fig = px.scatter(df, x='Voltage', y='Current',
                 title='Voltage vs Current High I-V Test',
                 labels={'Voltage': 'Voltage (V)', 'Current': 'Current (A)'},
                 hover_data=df.columns)

# Add a line trace
fig.add_trace(px.line(df, x='Voltage', y='Current').data[0])
