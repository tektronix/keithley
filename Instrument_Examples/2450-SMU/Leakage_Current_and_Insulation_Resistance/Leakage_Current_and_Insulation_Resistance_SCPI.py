...
instrument_write(s, ":SOUR:VOLT 20")                            # Set the source voltage to 20 V.
instrument_write(s, ":SOUR:VOLT:ILIMIT 0.01")                   # Set the current limit to 10 mA.
instrument_write(s, ":SENSE:FUNC \"CURR\"")                     # Set the instrument to measure current.
#instrument_write(s, ":ROUT:TERM REAR")                          # Set to use rear-panel terminals.
instrument_write(s, ":CURR:RANG:AUTO ON")                       # Set the current range to autorange.
instrument_write(s, ":SENSE:CURR:NPLC 1")                       # Set the number of power line cycles to 1.
instrument_write(s, ":TRIG:LOAD \"DurationLoop\", {0}, {1}".    # Load the Duration Loop trigger model to 
                 format(run_duration, sample_interval))         # run for 60 s at 200 ms intervals.
...
while j <= expected_readings_count:
    ...
    end_index = int(instrument_query(s, ":TRAC:ACT? \"defbuffer1\"", 16).rstrip())
    if end_index >= j:
        # Get the reading + relative timestamp pairs out one at a time and print to the display. 
        temp_list = instrument_query(s, ":TRAC:DATA? {0}, {1}, \"defbuffer1\", REL, READ".format(j, j),
                                     (16*2)).rstrip().split(',')
        print("{0:10} | {1:6.4f} | {2:3.4E}".format(j, float(temp_list[0]), float(temp_list[1])))
        j += 1

instrument_write(s, ":SOUR:VOLT 0")                             # Discharge the capacitor to 0 V.
