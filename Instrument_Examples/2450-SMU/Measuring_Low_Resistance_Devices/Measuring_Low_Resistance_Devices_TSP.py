...
instrument_write(s, "trigger.model.load(\"SimpleLoop\", 100)")                    # Configure Simple Loop trigger model template to make 100 readings.
instrument_write(s, "smu.measure.func = smu.FUNC_RESISTANCE")                                # Set to measure resistance.
instrument_write(s, "smu.measure.autorange = smu.ON")                            # Turn on auto range.
instrument_write(s, "smu.measure.offsetcompensation = smu.ON")                                 # Enable offset compensation.
instrument_write(s, "smu.measure.sense = smu.SENSE_4WIRE")                                 # Set to use 4-wire sense mode.
instrument_write(s, "display.changescreen(display.SCREEN_GRAPH_SWIPE)")                             # Show the GRAPH swipe screen.
instrument_write(s, "format.asciiprecision = 6")
...
readings_1 = instrument_query(s, "printbuffer(1, 50, defbuffer1.readings, defbuffer1.relativetimestamps)", (16*100*2)).rstrip()
readings_2 = instrument_query(s, "printbuffer(51, 100, defbuffer1.readings, defbuffer1.relativetimestamps)", (16*100*2)).rstrip()
