...
"""*********************************************************************************
	...
	This example application demonstrates the a more methodical means of scanning
	channels to capture multiple data points we refer to as pseudo scanning.
	Instead of a single trigger to execute the sequential scanning and looping
	control, the operator iterate over each individual channel capturing and
	exracting one reading at a time. 
            A. All channels of interest are configured for their measurement
               function with desired attributes.
            B. We size the internal data buffer to its minimum of 10. The goal, in
               this case, is to focus on a single reading at a time. 
            C. We loop over each channel of interest, define in a list.
                a. The channel is closed.
                b. The measurement is acquired via a READ?
                c. The reading is added to a comma-separated string
                d. The buffer is cleared ensure each new reading is the first in
                   the buffer.
            D. New data of the comma-separated string are added to a *.csv file.
            E. To control the scan interval such that that full set of channels
               is acquired each 30s, we inject some additional delay. The pseudo-
               scan of all channels takes approximately 20s, so 10s of delay
               is applied. 
...
instrument_write(my_daq, "TRAC:POIN 10, \"defbuffer1\"")
...
instrument_write(my_daq, "ROUTe:CSO (@ALLSLOTS), OFF")
...
for looper in range(0, 21600):             # If the loop iterates every nn seconds, calculate the stop point
    my_daq = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # Establish a TCP/IP socket object
    my_daq.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
    instrument_connect(my_daq, my_ip_address, my_port, 20000, do_instr_reset, do_instr_clear, do_instr_id_query)

    write_string = ""
    print("Loop #{0}".format(looper))

    for chan in channel_list:
        instrument_write(my_daq, "ROUT:CHAN:CLOS (@{0})".format(chan))
        my_val = instrument_query(my_daq, "READ?", 32).strip()
        if chan == "101":
            write_string = my_val
        else:
            write_string = write_string + "," + my_val

        instrument_write(my_daq, "TRAC:CLE")

    instrument_disconnect(my_daq)
    write_data(output_data_path, write_string)
    time.sleep(10.0)  # additional delay to make the scan interval 30s
