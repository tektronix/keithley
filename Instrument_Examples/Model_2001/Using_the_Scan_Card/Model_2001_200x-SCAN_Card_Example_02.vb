...
Module Module1
    ...
    Sub Main()
        ...
        Instrument_Write(my_instrument, "ROUT:SCAN:FUNC (@1:3), 'FRES'")
        Instrument_Write(my_instrument, "ROUT:SCAN:FUNC (@4:10), 'NONE'")       ' Option 1: if we want to exempt channels from the scan, we specify their function as none; otherwise they are included by default and set to DCV
        'Instrument_Write(my_instrument, "ROUT:SCAN (@1:3)")                    ' Option 2: if we don't include the line above, use this to specify a distinct scan list
        Instrument_Write(my_instrument, "TRIG:COUN 3")                          ' Number of channel measurements that are triggered in each scan, in this case 3 channels = 3 triggers
        Instrument_Write(my_instrument, "TRIG:SOUR IMM")
        Instrument_Write(my_instrument, "ARM:LAY2:COUN 10")                      ' Number of scans to execute
        Instrument_Write(my_instrument, "ARM:LAY2:SOUR BUS")
        Instrument_Write(my_instrument, "TRAC:CLE")
        Instrument_Write(my_instrument, "TRAC:POIN 30")
        ...
        Do While status_mon <> 512
            Instrument_Write(my_instrument, "*TRG")
            Threading.Thread.Sleep(500)  ' for the bus triggering that depends on the *TRG command, adjust your delay accordingly or you will witness error codes
